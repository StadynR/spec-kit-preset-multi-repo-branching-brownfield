...
This preset can be stacked with the [Brownfield extension](https://github.com/Quratulain-bilal/spec-kit-brownfield):
...
When both are installed, the preset replaces the four brownfield commands with multi-repo-aware versions. The brownfield workflow then:

1. Discovers independent repositories and Git submodules using `multi_repo_branching` configuration.
2. Preserves repository boundaries when generating the constitution, templates, and `AGENTS.md`.
3. Records affected repositories in `plan.md` using an `Affected Repositories` table.
4. Adds branch creation tasks for affected repositories at the start of Phase 1 in `tasks.md`.
5. Includes repository scope and branch coordination when migrating existing features.
